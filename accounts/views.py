...
from .forms import RegisterForms, UserForm, UserProfileForm
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # If the login attempt fails, handle the remaining attempts here
        messages.error(request, 'Invalid email or password.')
        attempts_remaining = 3
        if 'attempts_remaining' not in request.session:
            request.session['attempts_remaining'] = attempts_remaining
        else:
            request.session['attempts_remaining'] -= 1

        if request.session['attempts_remaining'] <= 0:
            messages.error(request, 'You have exceeded the maximum number of login attempts.')
            messages.error(request, 'You might have to reconsider changing your credentials.')
            return redirect('accounts:login')
        else:
            messages.error(request, f'You have {request.session["attempts_remaining"]} attempts remaining.')
